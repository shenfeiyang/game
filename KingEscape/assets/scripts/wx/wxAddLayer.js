...
cc.Class({
            ...
            if (config.gamecenter_link && cc.sys.platform == cc.sys.WECHAT_GAME) {
                ...
                if (global.showAuthLy) {
                    if (launcInfo.query['inviteFriend']) {
                        //群接口进入
                        if (parseInt(launcInfo.query['inviteGroup'])) {
                ...
                if (launcInfo && launcInfo['inviteFriend']) {
    ...
    *       self：{avatarUrl,city,gender,name,rank,score,titleName,}
     ...
     * @param {* fail } pointId 视频点

     */

    playAd(func1, func2, pointId = -1) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
            ...
            this._playAd(func1, func2, false, pointId);
        } else if (cc.sys.platform == cc.sys.QQ_PLAY) {
    ...
    _playAd(func1, func2, flag = false, pointId = -1) {
        if (!!!self._isPlay) {
            self._isPlay = true;
            wxAd.playAd(func1, func2, self, pointId);
            self._playAdTimes = 5;
    ...
    playAdNew(func1, flag = false, num = -1) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
            ...
            if (!config.isShowyd) {
                this._playAd(func1, null, flag, num);
            } else if (config.isShowyd == 1) {
                this._playAd(func1, func2, flag, num);
    ...
    */
     ...
     */
    goGiftShop(num = 0) {
        if (!this._giftShopNode) {
            cc.loader.loadRes('plugIn/giftShop/giftShop', cc.prefab, function (err, pre) {
                if (err) {
                    console.error(err);
                    return;
                }
                this._giftShopNode = cc.instantiate(pre);
                this.node.addChild(this._giftShopNode, 3000);
                this._giftShopNode.getComponent('giftShop').initData(num);
            }.bind(this))
        } else {
            this._giftShopNode.getComponent('giftShop').initData(num);
        }
    ...
    getLoginCfgByKey(key) {
        if (typeof key != 'string')
            return 0;
        return global.getLoginCfgByKey(key);
    },
    /**
     * 获取是否为 北上广深   返回 1（是）,0（否）
     */
    getCityFlag() {
        return this.getLoginCfgByKey('cityFlag');
    },
    /**
     * 获取垃圾用户（分享全部转换为视频） 返回 1， 其他为有效用户
     */
    getSpShare() {
        return this.getLoginCfgByKey('spshare');
    },
    /**
     * 点击分享小卡片进入的人  1 消除类游戏显示新的首页， 0 显示老的首页
     */
    getSpShareUser() {
        return this.getLoginCfgByKey('spShareUser');
    },
    /**
     * 是否为新用户 1 是， 0 不是
     */
    getNewUser() {
        return this.getLoginCfgByKey('newuser');
    },
    /**
     * 矩阵点击上报
     * @param {*} clickAppId 
    ...
    reportClickNew(clickAppId, name) {
        this._userOperate.reportClickNew(clickAppId, name);
    },
    /**
     * 获取周卡月卡时间
     * @param {*} callBack   返回 {ecode:0 remainDays:10}     remainDays 为剩余天数    remainDays = -1 没有卡    remainDays = 0 已过期
     * @param {*} tag   周卡（weekcard）| 月卡（monthcard）
     */
    getCard(callBack, tag = "weekcard") {
        this._userOperate.getCard(callBack, tag);
    },
    /**
     * 周卡月卡购买后上报
     * @param {*} callBack  返回 {ecode:0 remainDays:10} 周卡7天 月卡31天  注： 该接口  月卡和周卡 在有效时间内  返回剩余天数， 没有卡则创建   过期卡重置时间
     * @param {*} tag 周卡（weekcard）| 月卡（monthcard）
     */
    reportCard(callBack, tag = "weekcard") {
        this._userOperate.reportCard(callBack, tag);
    },
    /************************************************************
     * *****************************玩一玩功能********************
     * **********************************************************
