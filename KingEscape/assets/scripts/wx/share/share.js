...
export default class share {
  ...
  getShareTimeInfo() {
    ...
    if (!this._shareTimeInfo) {
      ...
      } else if (this._sharePolicy == 3) {
        this._shareTimeInfo = cc.sys.localStorage.getItem('shareTimeInfoPolicy_3');
      } else {
        this._shareTimeInfo = cc.sys.localStorage.getItem('shareTimeInfo');
      }
      //分享成功次数
      ...
      this._func_3 = () => {
        this._shareTimeInfo = {};
        this._shareTimeInfo['shaValStr_3'] = global.getUserDataInfoByName('shaValStr_3') || '2|4'; //失败变量 str解析 A 
        this._shareTimeInfo['shaFaiPro_3'] = global.getUserDataInfoByName('shaFaiPro_3') || '40|100';  // 分享失败概率 B
        let _failPro = this._shareTimeInfo['shaFaiPro_3'].split('|');
        this._shareTimeInfo['shaValArr_3'] = {};
        let _svs = this._shareTimeInfo['shaValStr_3'].split('|');
        let index = 0;
        _svs.forEach(element => {
          if (_failPro[index]) {
            that._shareTimeInfo['shaValArr_3'][parseInt(element)] = parseInt(_failPro[index]);
          } else {
            that._shareTimeInfo['shaValArr_3'][parseInt(element)] = 100;
          }
          index++;
        });
        this._shareTimeInfo['couSucTimes_3'] = 0; // 连续成功次数；
        cc.sys.localStorage.setItem('shareTimeInfoPolicy_3', JSON.stringify(this._shareTimeInfo));
      }
      //处理
      this._doFunc = () => {
        if (this._sharePolicy == 1) {
 ...
 */
   ...
   * @param {*} flag 
   */
  updSucShaTimCouTimes_3(flag = false) {
    // 分享成功
    if (flag) {
      //连续成功次数
      this._shareTimeInfo['couSucTimes_3'] += 1;
    } else {   // 分享失败
      ///连续成功次数 归零
      this._shareTimeInfo['couSucTimes_3'] = 0;
    }
    cc.sys.localStorage.setItem('shareTimeInfoPolicy_3', JSON.stringify(this._shareTimeInfo));
  }
  /**
  ...
  gameEvent() {
    ...
    if (cc.sys.platform == cc.sys.WECHAT_GAME) {
      ...
      wx.onShow(function (res) {
        //console.log("+++++++++++++++++++++++shareMe", res, wx.getLaunchOptionsSync())
  ...
  */
  ...
  _policyFunc_3() {
    let flag = true;
    if (this._shareTimeInfo['shaValArr_3'][this._shareTimeInfo.couSucTimes_3 + 1]) {
      //概率判定
      let rand = Math.floor(Math.random() * 100);
      if (rand <= this._shareTimeInfo['shaValArr_3'][this._shareTimeInfo.couSucTimes_3 + 1]) {
        this._showModal();
        flag = false;
      }
    }
    //更新数据
    this.updSucShaTimCouTimes_3(flag);
    return flag;
  };
  /**
   * 处理分享事件
  ...
  updateShareMenu = function () {
    if (cc.sys.platform == cc.sys.WECHAT_GAME) {
      ...
      let activityId = global.getUserDataInfoByName('shareActiveId');
      if (activityId) {
        console.log("updateShareMenu activityId", activityId)
        wx.updateShareMenu({
          ...
          complete: function (res) {
            console.log("updateShareMenu complete res:", res)
          },
        })
      } else {
        wx.updateShareMenu({
  ...
  _shareAppMessage(callBack = false, num = -1, score = false, isSpecial = false, obj = false, isPeople = false, gold = false, wxAddLayer = false,
    ...
    wx.shareAppMessage({
      ...
      cancel: res => {
        console.log("shareAppMessage cancel", res)
        self._cancel = true;
      }
    });
  };
