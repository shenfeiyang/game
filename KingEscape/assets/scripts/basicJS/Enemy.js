...
cc.Class({
    ...
    //碰到边界转向
    borderChangeRotation(enemy) {
        let turn_time = parseFloat(this.roleData.get(enemy.level).turn_time);
        let rotationAction = cc.rotateTo(turn_time, this.food.getNearFood(enemy));
        enemy.getChildByName('enemy').runAction(rotationAction);
    },

    //攻击范围检测
    attackRangeCheck(enemy) {
        if (!this.stage.playerIsGod && this.game.gameState === 'game' && this.player.node.active === true) {
        ...
        switch (enemy.state) {
            ...
            case 'escape'://逃跑
                ...
                if (Math.random() < this.roleData.get(enemy.level).accelerate_pro) {
                    ...
                    //加速特效
                    let body = enemy.getChildByName('enemy').getChildByName('enemyBody');
                    let speedDragonBone = cc.instantiate(this.game.dragonBone);
                    speedDragonBone.getComponent('dragonBonesControll').yanwu();
                    body.addChild(speedDragonBone);
                    speedDragonBone.angle = -90;
                    speedDragonBone.y = speedDragonBone.height / 2 + body.height / 2;
                }
