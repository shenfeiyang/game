cc.Class({
    ...
    initAllPosAndSize(level) {
        ...
        //设置角色速度
        this.stage.moveSpeed = parseInt(data.move_speed) * (1 + this.game.buff2);

        //获取角色图片资源
        // 加载 SpriteFrame
        let roleSp = this.stage.findRoleSp(role);
        this.playerBody.getComponent(cc.Sprite).spriteFrame = roleSp;
        this.playerBody.width = data.body_width;
        ...
        this.sword.height = data.weapon_length * (1 + this.game.buff1);//buff加成
        this.sword.width = this.sword.height / weaponSp.getOriginalSize().height * weaponSp.getOriginalSize().width;
    ...
    slashAction() {
        ...
        //攻击速度
        let attack_speed = parseFloat(this.roleData.get(this.level).attack_speed) * (1 - this.game.buff3);
