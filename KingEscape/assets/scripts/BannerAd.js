...
const border = 5;
const space = 5;
const dsgSize = config.officalAccount ? config.officalAccount : { width: 640, height: 1136 };
const dp = 3.51;
...
cc.Class({
    ...
    replace: function () {
        ...
        if (!(bannerWidth <= 0 || this._fullWidth) && config.bannerLowest)
            bannerAd.onResize(function (res) {
                bannerAd.style.left = (screen.screenWidth - res.width) / 2 + self._reSize.width;
                bannerAd.style.top = screen.screenHeight - res.height - (!!_fullScreen ? 30 : 0) + self._reSize.height;
    ...
    create: function () {
        ...
        if (!wxBversionLess("2.0.4")) {
            ...
            if (typeof bannerId == 'string') {
                _bannerAd = bannerId;
            } else if (typeof bannerId == 'object') {
                let index = global.getUserDataInfoByName('bannerAdIndex');
                if (typeof index != 'number') {
                    index = Math.floor(Math.random() * bannerId.length * 10) % bannerId.length;
                }
                _bannerAd = bannerId[index];
                index += 1;
                if (bannerId.length <= index) {
                    index = 0;
                }
                global.setUserDataInfoByName('bannerAdIndex', index);
                console.log("createBannerId", _bannerAd, index);
            }
    ...
    resumeAdResize() {
        ...
        if (this._bannerAd) {
            let bannerAd = this._bannerAd;
            const _fullScreen = this._fullScreen;
            const screen = wx.getSystemInfoSync();
            if (!(bannerWidth <= 0 || this._fullWidth) && config.bannerLowest)
                this._bannerAd.style.top = screen.screenHeight - this._bannerAd.style.realHeight - (!!_fullScreen ? 30 : 0);//BannerSize.height;
            else
    ...
    refresh: function (_, force) {
        ...
        const bannerNode = this._baseBannerAd ? this['bannerNodeBase'] : (bannerType ? this['bannerNode'] : this['bannerNodeBase']);
